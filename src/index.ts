...
async function getDependencySizes(packageJsonPath: string): Promise<void> {
  ...
  console.log(chalk.cyan.bold("Package Name") + "  |  " + chalk.cyan.bold("Unpacked Size (KB)"));
  ...
  for (const [dep, version] of Object.entries(dependencies)) {
    try {
      ...
      const sizeDisplay = sizeInKB.toFixed(2) + " KB";
  ...
  console.log(chalk.cyan("-".repeat(40)));
  console.log(chalk.bold(`Total Unpacked Size: ${chalk.magenta.bold((totalUnpackedSize / 1024).toFixed(2))} KB`));
}
